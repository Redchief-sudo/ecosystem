...
logger = logging.getLogger('rpc_ping_test')
...
class RPCPingTester:
    ...
    def get_rpc_endpoints(self) -> Dict[str, List[str]]:
        ...
        for chain_config in networks:
            ...
            # Primary RPC
            rpc_url = chain_config.get('rpc') or chain_config.get('rpc_url') or chain_config.get('rpc_primary')
            if rpc_url:
                # Substitute API keys
            ...
            # Handle rpc_fallback_1, rpc_fallback_2, etc.
            for i in range(1, 4):  # Support up to 3 fallbacks
                fallback_key = f'rpc_fallback_{i}'
                if fallback_key in chain_config:
                    fallback_url = self._substitute_api_keys(chain_config[fallback_key])
                    if fallback_url:
                        chain_endpoints.append(fallback_url)
