...
class EliteStrategyManager:
    ...
    async def initialize(self) -> None:
        """Initialize the strategy manager and all strategies."""
        logger.info("Initializing EliteStrategyManager...")
        
        # Initialize individual strategies if they have initialize method
        for strategy in self.strategies:
            if hasattr(strategy, 'initialize') and callable(getattr(strategy, 'initialize')):
                try:
                    await strategy.initialize()
                    logger.info(f"Strategy {self._get_strategy_name(strategy)} initialized successfully")
                except Exception as e:
                    logger.error(f"Failed to initialize strategy {self._get_strategy_name(strategy)}: {e}")
        
        logger.info("EliteStrategyManager initialization complete")
