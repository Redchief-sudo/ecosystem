...
class RiskCapsStrategy(BaseStrategy):
    ...
    def __init__(self, *args, **kwargs):
        # Extract trade_history_tracker before calling super().__init__
        trade_history_tracker = kwargs.pop('trade_history_tracker', None)
        ...
        # Initialize trade history tracker for data-driven probability estimation
        self.trade_history_tracker: TradeHistoryTracker = trade_history_tracker or TradeHistoryTracker(min_trades_for_statistics=10)
        
        logger.info(f"[{self.STRATEGY_NAME}] Initialized and ready (with trade history tracking).")
    ...
    async def evaluate_token(self, token: Dict[str, Any]) -> Optional[TradeSignal]:
        try:
            ...
            # Get symbol for historical data lookup
            symbol = token.get("symbol", token.get("token_symbol", "unknown"))
            
            sizing_result = await self._calculate_optimal_position(market_data, risk_metrics, config, symbol=symbol)
            if not sizing_result or sizing_result.size < config["min_position_size"]:
                return None

            return await self._generate_signal(token, market_data, sizing_result, risk_metrics, symbol=symbol)
    ...
    async def _calculate_risk_metrics(self, token: Dict[str, Any], market_data: Dict[str, float], config: Dict[str, Any]) -> Optional[RiskMetrics]:
        try:
            ...
            # Try to get returns history if available, otherwise use simplified estimate
            returns_history = token.get("returns_history", [])
            sharpe = self._estimate_sharpe_ratio(returns_history=returns_history, drawdown=drawdown, volatility=volatility)
    ...
    def _calculate_var(price: float, volatility: float, confidence: float) -> float:
        """Calculate VaR with proper z-scores from scipy (institutional-grade)."""
        return RiskMetricsCalculator.calculate_var(price, volatility, confidence)

    @staticmethod
    def _estimate_sharpe_ratio(returns_history: Optional[List[float]] = None, drawdown: Optional[float] = None, volatility: Optional[float] = None) -> float:
        """
        Calculate proper Sharpe ratio from returns history (institutional-grade).
        
        If returns_history is provided, uses proper Sharpe calculation.
        Otherwise falls back to simplified estimate using drawdown and volatility.
        """
        if returns_history and len(returns_history) >= 2:
            # Use proper Sharpe calculation from returns
        ...
        # Fallback to simplified estimate if no returns history
    ...
    async def _calculate_optimal_position(
        ...
        """
        Calculate optimal position size using Decimal precision (institutional-grade).
        
        Args:
            market_data: Market data dictionary
            risk_metrics: Risk metrics for the token
            config: Strategy configuration
            symbol: Optional token symbol for historical data lookup
        """
        try:
            # Use Decimal precision for all calculations
            price = FinancialDecimal(market_data["price"])
            win_prob = FinancialDecimal(str(self._estimate_win_probability(risk_metrics, symbol=symbol)))
            win_loss_ratio = FinancialDecimal("2.0")
            
            # Kelly Criterion with Decimal precision
            kelly_full = max(
                FinancialDecimal("0.0"),
                (win_prob * win_loss_ratio - (FinancialDecimal("1.0") - win_prob)) / win_loss_ratio
            )
            kelly_fraction = kelly_full * FinancialDecimal(str(config["kelly_fraction"]))
            
            # Risk adjustments with Decimal precision
            vol_adj = FinancialDecimal("1.0") / (FinancialDecimal("1.0") + FinancialDecimal(str(risk_metrics.volatility)))
            dd_adj = FinancialDecimal("1.0") - (FinancialDecimal(str(risk_metrics.drawdown)) / FinancialDecimal(str(config["max_drawdown"])))
            liq_adj = FinancialDecimal(str(risk_metrics.liquidity_score))
            
            # Position size calculation
            raw_size = kelly_fraction * vol_adj * dd_adj * liq_adj
            
            # Apply bounds
            min_size = FinancialDecimal(str(config["min_position_size"]))
            max_size = FinancialDecimal(str(config["max_position_size"]))
            size = max(min_size, min(raw_size, max_size))
            
            # Risk-adjusted size
            var_95 = FinancialDecimal(str(risk_metrics.var_95))
            max_risk_size = (FinancialDecimal(str(config["max_position_risk"])) * price) / var_95
            risk_adjusted = min(size, max_risk_size)
            
            # Stop loss and take profit
            stop_loss = price * (FinancialDecimal("1.0") - FinancialDecimal(str(config["max_drawdown"])))
            take_profit = price * (FinancialDecimal("1.0") + FinancialDecimal("2.0") * FinancialDecimal(str(config["max_drawdown"])))
            
            # Convert back to float for return (maintains compatibility)
            return PositionSizingResult(
                size.to_float(),
                self._calculate_confidence(risk_metrics, size.to_float(), config),
                kelly_fraction.to_float(),
                risk_adjusted.to_float(),
                stop_loss.to_float(),
    ...
    def _estimate_win_probability(self, risk_metrics: RiskMetrics, symbol: Optional[str] = None) -> float:
        """
        Estimate win probability using historical trade data (institutional-grade).
        
        Uses actual win rate from trade history if available, otherwise falls back
        to heuristic based on risk metrics.
        
        Args:
            risk_metrics: Risk metrics for the token
            symbol: Optional token symbol for historical lookup
        
        Returns:
            Win probability (0.05-0.95)
        """
        # Try to get historical win rate first
        if symbol:
            ...
            # If we have sufficient data (not default 0.5), use it
            if win_rate != 0.5 or self.trade_history_tracker.get_trade_count(
                strategy=self.STRATEGY_NAME,
                symbol=symbol
            ) >= self.trade_history_tracker.min_trades:
                logger.debug(
                    f"Using historical win rate for {symbol}: {win_rate:.2%} "
                    f"({self.trade_history_tracker.get_trade_count(strategy=self.STRATEGY_NAME, symbol=symbol)} trades)"
                )
                return min(max(win_rate, 0.05), 0.95)
        
        # Fallback to heuristic if insufficient historical data
        logger.debug(
            f"Insufficient historical data for {symbol or 'unknown'}, "
            f"using heuristic win probability"
        )
    ...
    async def _generate_signal(
        ...
        win_prob = self._estimate_win_probability(risk_metrics, symbol=symbol)
        if win_prob > 0.6:
            side = SignalType.BUY
    ...
    def _create_signal(self, signal_type, confidence, price, size, stop_loss, take_profit, token_address=None, token_symbol=None, reason=None, metadata=None):
        ...
        score = float(confidence) * (1 + float(size))
        return TradeSignal(
