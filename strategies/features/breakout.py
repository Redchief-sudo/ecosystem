...
class EliteBreakoutStrategyV2(BaseStrategy):
    ...
    # ------------------------------------------------------------------
    def evaluate(self, market_state: Dict, context: Optional[Dict] = None) -> Optional[StrategyDecision]:
        """Synchronous evaluation not used – strategy logic lives in evaluate_token."""
        return None

    # ------------------------------------------------------------------
    # MAIN EVALUATION ENTRYPOINT
    # ------------------------------------------------------------------
    async def evaluate_token(self, token_data: Dict) -> Optional[TradeSignal]:
