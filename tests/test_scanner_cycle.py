...
async def initialize_network_manager():
    try:
        ...
        print(f"⚠️ Failed to initialize NetworkManager: {e}")
...
async def test_individual_scanners():
    ...
    try:
        ...
        dex_scanner = DexScreenerScanner(config=dex_config, network_manager=NETWORK_MANAGER)
        ...
        mempool_scanner = MempoolScanner(config=mempool_config, network_manager=NETWORK_MANAGER)
...
async def main():
    ...
    network_ok = individual_ok = director_ok = False
    try:
